#!/usr/bin/env bash
set -euo pipefail
echo 'Restoring the Vercel serverless adapter (api/index.ts, vercel.json)...'

mkdir -p "$(dirname 'api/index.ts')"
cat > 'api/index.ts' << 'VERCEL_FIX_EOF'
...
mkdir -p "$(dirname 'server/_core/app.ts')"
cat > 'server/_core/app.ts' << 'VERCEL_FIX_EOF'
...
VERCEL_FIX_EOF
echo 'Wrote server/_core/app.ts'

mkdir -p "$(dirname 'vercel.json')"
cat > 'vercel.json' << 'VERCEL_FIX_EOF'
{
...
echo 'Done. Next: git add -A, git commit, git push.'
